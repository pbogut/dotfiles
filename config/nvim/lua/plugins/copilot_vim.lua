...
local function accept_one_line()
  ...
  local outdent = vim.api.nvim_replace_termcodes(
    vim.fn['repeat']("<left><del>", s.outdentSize), true, false, true
  )
  local delete = vim.api.nvim_replace_termcodes(
    vim.fn['repeat']("<del>", s.deleteSize), true, false, true
  )
  ...
  if lines[1]:len() == 0 then
    local fix_indent = vim.api.nvim_replace_termcodes(
      "<cr><c-w>", true, false, true
    )
    if lines[3] == nil then
